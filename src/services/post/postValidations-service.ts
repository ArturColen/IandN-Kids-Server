...
export const validatePostData = (postData: PostInterface) => {
    ...
    if (!postData.author) {
        throw new Error(
            'Favor preencher corretamente o nome do autor da postagem.'
        );
    } else if (typeof postData.author !== 'string') {
        throw new Error(
            'O nome do autor da postagem deve ser uma string válida.'
        );
    ...
    if (!postData.content) {
        ...
        throw new Error(
            'O conteúdo da postagem deve ter pelo menos 100 caracteres.'
        );
    }

    if (!postData.imageLink) {
        throw new Error(
            'Favor preencher corretamente o link da imagem da postagem.'
        );
    } else if (typeof postData.imageLink !== 'string') {
        throw new Error(
            'O link da imagem da postagem deve ser uma string válida.'
        );
